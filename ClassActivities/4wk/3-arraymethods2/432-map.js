 ...
 const data = [
  ...
  const cityyyy=data.map(v =>{return  v.address.city});

  console.log(cityyyy);
  // create an array of latitude and longitude coordinate pairs
  const coordies=data.map(v=>{return[v.address.geo.lat,v.address.geo.lng]})
  console.log(coordies);
  // create an array of strings that combine the company's catchPhrase and bs
  const dreamCatcher=data.map(v=>{return(`${v.company.catchPhrase} & ${v.company.bs}`)})
