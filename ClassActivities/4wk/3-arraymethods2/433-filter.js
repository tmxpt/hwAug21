...
const data = [
  ...
  const incompleterPeter=data.filter(v=>v.completed!=true);
  console.log(incompleterPeter,`\n INCOMPLETE TASKS ABOVE, VOTE TO EJECT IMPOSTER\n`);
  // filter for user 6's tasks
  const sixthCoyote=data.filter(v=>v.userId===6);
  console.log(sixthCoyote,`\n USER 6 TASKS ABOVE`);
  // filter for tasks with a title less than 24 characters long
  const characterLimit=data.filter(v=>v.title.length<15);
  console.log(characterLimit,`\n SHORT TASKS ABOVE, VERY SHORT LIKE KEVIN HART`);